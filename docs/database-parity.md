...
| `user_account` | `egas.UserAccount` in `auth.cds` | Provider and bootstrap only; no generic account endpoint. Passwords are Argon2id hashes. | Failure-count check; unique username/staff identifier. |
| `user_account_role` | `egas.UserAccountRole` | Session middleware authorizes exactly `AuthSession.activeRole`; roles are never unioned. Admin role-management service comes later. | Role/admin-only/self-grant/self-revoke checks and active lookup index. |
| `auth_session` | `egas.AuthSession` | Only token hashes are queried; active account + one active selected role + idle/absolute expiry required. | Active-role/expiry checks and active-session index. |
| `auth_login_attempt` | `egas.AuthLoginAttempt` | Durable login evidence boundary exists; login/rate-limit endpoint is next auth phase. | Identifier/IP time indexes. Uses CAP UUID rather than baseline `bigserial`; identity semantics are unchanged. |
| `security_event` | `egas.SecurityEvent` in `audit-documents.cds` | Security logging integration is next auth/admin phase. | JSON object check and type/actor indexes. |
...
| `approving_authority_assignment` | `egas.ApprovingAuthorityAssignment` in `authority.cds` | Authorization by assignment/delegation is a later workflow action concern; job-title text never grants access. | Partial unique active-primary-per-unit index, date/kind/no-self-config checks. |
| `authority_delegation` | `egas.AuthorityDelegation` | Admin delegation endpoint is later; delegation remains assignment-scoped. | Date check and trigger rejecting delegation to the primary authority itself. |
