...
async function main(): Promise<void> {
  ...
  if (!batch || !operator) throw new Error('Usage: npm run data:revalidate -- --batch <UUID> --operator <username>')
  const service = new ImportService(getPool(loadConfig()))
  const actor = await service.operator(operator)
  const result = await service.revalidate(batch, actor, {
    ipAddress: null, userAgent: 'egas-data-revalidate-cli', correlationId: randomUUID()
  })
