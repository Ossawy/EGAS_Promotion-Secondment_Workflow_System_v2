...
const textExtensions = new Set(['.csv', '.js', '.json', '.md', '.mjs', '.sql', '.ts', '.txt', '.yaml', '.yml', ''])
const patterns = [
...
async function walk(directory) {
  for (const entry of await readdir(directory, { withFileTypes: true })) {
    ...
    for (const pattern of patterns) if (pattern.regex.test(content)) findings.push(`${path.relative(repositoryRoot, fullPath)}: ${pattern.name}`)
  }
}
await walk(repositoryRoot)
if (findings.length) { console.error(findings.join('\n')); process.exitCode = 1 }
else console.info('No supported secret signatures found in the repository worktree.')
