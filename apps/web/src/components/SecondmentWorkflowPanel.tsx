...
const errorLabels: Record<string, string> = {
  ...
  'يلزم تحديد الفئة الوظيفية وتقرير آخر ترقية وإدخال وظيفة مقترحة كاملة واحدة على الأقل لكل عامل.',

WORKFLOW_SELECTION_INCOMPLETE:
  'يجب اختيار وظيفة مقترحة واحدة بالضبط لكل عامل.',
...
export function SecondmentWorkflowPanel({ detail }: { detail: WorkflowRequestDetail }): React.JSX.Element {
  ...
  try {
    await apiJson(
      `/api/workflow/requests/${detail.id}/secondment/candidates/${candidateId}/preparation`,
      'PUT',
      {
        jobCategoryCode:
          data.get(
  ...
  return (
  <section className="panel secondment-panel">
    <div className="panel__header">
      <div>
        <h2>
          نموذج الندب · المرحلة {detail.currentStage}
        </h2>

        <p>
          الوظائف المقترحة والاختيار محفوظة داخل الدورة{' '}
    ...
    {detail.candidates.length === 0 ? (
      <EmptyState
        ...
        body="لا يمكن إعداد وظائف الندب قبل إضافة العاملين في S1."
      />
    ) : (
      <div className="candidate-position-list">
        {detail.candidates.map(candidate => {
          const options =
            byCandidate.get(candidate.id) ?? []
          ...
          return (
            ...
            >
              <header>
                ...
                <span>
                  {options.length} وظيفة مقترحة
                </span>
              </header>

              {detail.currentStage === 'S2' &&
                actionable && (
                  <form
                    className="candidate-category"
                    onSubmit={event =>
  ...
  onChange={event =>
        ...
        [candidate.id]:
          event.target.value
              ...
              {!(
                ...
                <div className="candidate-category candidate-category--readonly">
                  <span>
                    الفئة الوظيفية:{' '}
                    <strong>
                      {candidate.formSection
                ...
                <div className="position-options">
                  {options.map(option => (
                    <label
                      ...
                      className={
                        option.isSelected
                          ? 'position-option position-option--selected'
                          : 'position-option'
                      }
                    >
                      {detail.currentStage ===
                      ...
                      <span>
                        ...
                        <small>
                          {
                            option.organizationalDependency
                          }
                          {' · '}
                          {option.qualificationStatus ===
                          'QUALIFIED'
                            ? 'مستوفي'
                            : 'غير مستوفي'}
                        </small>
    ...
    {actionable &&
      actionByStage[detail.currentStage] && (
        <footer className="form-actions secondment-actions">
          {(detail.currentStage === 'S1' ||
            detail.currentStage === 'S2') && (
            <span>
              يتحقق الخادم من وجود التوقيع
              الإلزامي قبل الانتقال.
            </span>
          )}
